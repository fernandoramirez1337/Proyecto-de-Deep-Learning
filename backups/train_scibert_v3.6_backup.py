...
def main():
    ...
    # Configuración V3.6 - Aggressive cs.AI Focus
    # V3 tuvo mejor cs.AI recall (26.22%), seguir esa dirección
    FREEZE_BERT_LAYERS = 3      # Menos que V3 (era 4) - más capacidad
    DROPOUT = 0.35              # Menos que V3 (era 0.4) - menos restricción
    BATCH_SIZE = 12             # Optimizado para M2
    EPOCHS = 10
    LR = 5e-5                   # Igual que V3
    WEIGHT_DECAY = 0.01         # Igual que V3
    PATIENCE = 3
    USE_CLASS_WEIGHTS = True
    AGGRESSIVE_CS_AI = True     # NUEVO: Peso agresivo para cs.AI
    NUM_WORKERS = 0
    ...
    if USE_CLASS_WEIGHTS:
        if AGGRESSIVE_CS_AI:
            # Pesos agresivos: cs.AI x3 (orden: cs.AI, cs.CL, cs.CV, cs.LG)
            class_weights = np.array([3.0, 1.0, 1.0, 1.0], dtype=np.float32)
            print(f" Class weights (AGGRESSIVE): cs.AI=3.0, others=1.0")
        else:
