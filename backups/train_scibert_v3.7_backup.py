...
def main():
    ...
    if USE_CLASS_WEIGHTS:
        if AGGRESSIVE_CS_AI:
            # Pesos ajustados para cs.AI (orden: cs.AI, cs.CL, cs.CV, cs.LG)
            class_weights = np.array([CS_AI_WEIGHT, 1.0, 1.0, 1.0], dtype=np.float32)
            print(f" Class weights: cs.AI={CS_AI_WEIGHT}, others=1.0")
        else:
