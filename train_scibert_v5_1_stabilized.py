"""
Training SciBERT V5.1: STABILIZED Cross-Attention + Back-Translation

PROBLEM IDENTIFIED IN V5.0:
- Epoch 1: 57.60% val acc (PEAK)
- Epoch 2: 46.84% val acc (CRASH -10.76%)
- Root cause: LR too high + Class weights too aggressive with augmented data

STABILIZATION FIXES:
- LOWER LR: 5e-5 → 3e-5 (40% reduction, prevent oscillation)
- SOFTER CLASS WEIGHTS: 2.0 → 1.4 (30% reduction, less aggressive)
- KEEP WARMUP: Linear warmup (10% of steps) prevents early overfitting
- SLIGHTLY LOWER DROPOUT: 0.35 → 0.30 (improve capacity)

EXPECTED RESULTS:
- Stable training (no crash)
- Better cs.LG recall (was 37.33% in V5.0)
- Maintain cs.AI recall > 35%
- Target: 59-60% test accuracy

CONFIGURATION:
- Architecture: CrossAttentionSciBERT (same as V5.0)
- Dataset: data/arxiv_papers_augmented.csv (same as V5.0)
- FREEZE_LAYERS: 3
- DROPOUT: 0.30 (was 0.35)
- CLASS_WEIGHTS: [1.4, 1.0, 1.0, 1.0] (was [2.0, 1.0, 1.0, 1.0])
...
class OptimizedTrainer:
    ...
    def train(self, train_loader, val_loader, epochs=10):
        ...
        print("="*70)
        print("TRAINING V5.1: STABILIZED Cross-Attention + Back-Translation")
        print("="*70)
        print(f"\nEpochs: {epochs}")
...
def plot_history(history, save_path='scibert_v5_1_history.png'):
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('scibert_v5_1_confusion.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("\n✓ Confusion matrix saved: scibert_v5_1_confusion.png")

    # Save model
    torch.save(model.state_dict(), 'best_scibert_v5_1_stabilized.pth')
    print("✓ Model saved: best_scibert_v5_1_stabilized.pth")
    ...
    print(f"\n{'Metric':<20} {'Baseline':<12} {'V5.1':<12} {'Improvement'}")
    print("-"*60)
